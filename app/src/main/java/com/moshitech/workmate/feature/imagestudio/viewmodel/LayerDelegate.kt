...
class LayerDelegate(
    ...
    fun addTextLayer(text: String, x: Float, y: Float, color: Int) {
        ...
        val newLayer = TextLayer(text = text, x = x, y = y, color = color, zIndex = nextZIndex)
        _uiState.update {
            it.copy(
    ...
    fun updateTextProperty(
        ...
        _uiState.update { state ->
            val updatedLayers =
                    state.textLayers.map { layer -> if (layer.id == id) update(layer) else layer }
    ...
    fun duplicateTextLayer(id: String) {
        ...
        val newLayer =
                layerToDuplicate.copy(
                        id = UUID.randomUUID().toString(),
                        x = layerToDuplicate.x + 40f,
                        y = layerToDuplicate.y + 40f
                )
        _uiState.update {
            it.copy(textLayers = it.textLayers + newLayer, selectedTextLayerId = newLayer.id)
        }
        onApplyNeeded(true)
    }
    ...
    fun enterTextEditMode(id: String) {
        ...
        _uiState.update {
            it.copy(selectedTextLayerId = id, editingTextLayerId = id, showFloatingToolbar = false)
        }
    }
    ...
    fun addSticker(resId: Int = 0, text: String? = null): String {
        ...
        // Initialize scale so the sticker appears at a normal size (100%) on SCREEN
        // regardless of how large/small the underlying bitmap is.
        // bitmapScale maps Bitmap Pixels -> Screen Pixels.
        // We want (BaseStickerSize * scale * bitmapScale) ~= BaseStickerSize (visually).
        // So scale should be 1 / bitmapScale.
        val containerW = _uiState.value.containerWidth.toFloat()
        val containerH = _uiState.value.containerHeight.toFloat()
        val bmpW = bitmap?.width?.toFloat() ?: 1f
        val bmpH = bitmap?.height?.toFloat() ?: 1f

        val scaleW = if (bmpW > 0) containerW / bmpW else 1f
        val scaleH = if (bmpH > 0) containerH / bmpH else 1f
        // Use the same Min-Scale logic as PhotoEditorViewModel.getBitmapScale()
        val bitmapScale = kotlin.math.min(scaleW, scaleH).coerceAtLeast(0.001f)
        val initialScale = 1f / bitmapScale

        // CRITICAL FIX: Convert CENTER coordinates to TOP-LEFT coordinates
        // The UI and CompositeRenderer treat layer.x/layer.y as TOP-LEFT of the sticker box
        // Base sticker size is 100dp in bitmap coordinates (100 / bitmapScale)
        val baseSizeInBitmapCoords = 100f / bitmapScale
        val stickerHalfWidth = (baseSizeInBitmapCoords * initialScale) / 2f
        val stickerHalfHeight = (baseSizeInBitmapCoords * initialScale) / 2f

        val topLeftX = centerX - stickerHalfWidth
        val topLeftY = centerY - stickerHalfHeight

        val newSticker =
                StickerLayer(
                        resId = resId,
                        text = text,
                        x = topLeftX, // Now using TOP-LEFT coordinates
                        y = topLeftY, // Now using TOP-LEFT coordinates
                        scaleX = initialScale,
                        scaleY = initialScale,
                        zIndex = nextZIndex
                )
    ...
    fun flipSticker(id: String, vertical: Boolean = false) {
        _uiState.update { state ->
            state.copy(
                    stickerLayers =
                            state.stickerLayers.map { layer ->
                                if (layer.id == id && !layer.isLocked) {
                                    if (vertical) layer.copy(isFlippedV = !layer.isFlippedV)
                                    else layer.copy(isFlipped = !layer.isFlipped)
                                } else layer
                            }
            )
        }
    ...
    fun rotateSticker90(id: String, clockwise: Boolean) {
        _uiState.update { state ->
            state.copy(
                    stickerLayers =
                            state.stickerLayers.map { layer ->
                                if (layer.id == id && !layer.isLocked) {
                                    layer.copy(
                                            rotation =
                                                    layer.rotation + (if (clockwise) 90f else -90f)
                                    )
                                } else layer
                            }
            )
    ...
    fun updateStickerLayer(id: String, transform: (StickerLayer) -> StickerLayer) {
        _uiState.update { state ->
            state.copy(
                    stickerLayers =
                            state.stickerLayers.map { layer ->
                                if (layer.id == id && !layer.isLocked) transform(layer) else layer
                            }
            )
        }
        onApplyNeeded(true)
    ...
    fun addShapeLayer(type: ShapeType): String {
        ...
        val width =
                if (type == ShapeType.LINE || type == ShapeType.ARROW) shapeSize * 0.8f
                else shapeSize
        ...
        val y =
                centerY -
                        (if (type == ShapeType.LINE || type == ShapeType.ARROW) shapeSize / 2f
                        else height / 2f)
        ...
        val newShape =
                ShapeLayer(
                        ...
                        width = width,
                        height =
                                if (type == ShapeType.LINE || type == ShapeType.ARROW) shapeSize
                                else height,
                        color = 0xFFCCCCCC.toInt(), // Light Gray Default
                        strokeWidth =
                                0f, // No stroke by default if filled, or small? PicsArt usually
                        // filled.
        ...
        android.util.Log.d(
                "ShapeDebug",
                "addShapeLayer: selectedShapeLayerId = ${newShape.id}, shapeLayers.size = ${_uiState.value.shapeLayers.size}"
        )
        onApplyNeeded(true)
        return newShape.id
    }
    ...
    fun updateShapeType(id: String, newType: ShapeType) {
        _uiState.update { state ->
            ...
            // Recalculate dimensions if switching between Line/Arrow and Box shapes to avoid
            // distortion
            ...
            if (isLinear != wasLinear) {
                ...
                val size =
                        if (bitmap != null)
                                minOf(bitmap.width.toFloat(), bitmap.height.toFloat()) / 2f
                        else 300f
                if (isLinear) {
                    newW = size * 0.8f
                    newH = 20f
            ...
            state.copy(
                    shapeLayers =
                            state.shapeLayers.map {
                                if (it.id == id)
                                        it.copy(type = newType, width = newW, height = newH)
                                else it
                            }
            )
        }
    ...
    fun duplicateShape(id: String): String {
        ...
        _uiState.update {
            it.copy(shapeLayers = it.shapeLayers + newShape, selectedShapeLayerId = newShape.id)
        }
        onApplyNeeded(true)
        return newShape.id
    }

    fun updateShapeLayer(
            id: String,
            saveHistory: Boolean = true,
            update: (ShapeLayer) -> ShapeLayer
    ) {
        _uiState.update { state ->
            state.copy(
                    shapeLayers =
                            state.shapeLayers.map { layer ->
    ...
    fun lockLayer(id: String, locked: Boolean) {
        _uiState.update { state ->
            ...
            val stickers =
                    state.stickerLayers.map { if (it.id == id) it.copy(isLocked = locked) else it }
            val shapes =
                    state.shapeLayers.map { if (it.id == id) it.copy(isLocked = locked) else it }
    ...
    fun deleteLayer(id: String, type: LayerType) {
        _uiState.update { state ->
            when (type) {
                ...
                LayerType.STICKER ->
                        state.copy(stickerLayers = state.stickerLayers.filter { it.id != id })
                LayerType.SHAPE ->
                        state.copy(shapeLayers = state.shapeLayers.filter { it.id != id })
                else -> state
            }
        }
        onApplyNeeded(true)
                        ...
                        state.copy(
                                textLayers =
                                        state.textLayers.map {
                                            if (it.id == id) it.copy(isVisible = !it.isVisible)
                                            else it
                                        }
                        )
                LayerType.STICKER ->
            ...
            when (type) {
                LayerType.TEXT ->
                        state.copy(
                                textLayers =
                                        state.textLayers.map {
                                            if (it.id == id) it.copy(zIndex = newZIndex) else it
                                        }
                        )
                LayerType.STICKER ->
                        state.copy(
            ...
            when (type) {
                LayerType.TEXT ->
                        state.copy(
                                textLayers =
                                        state.textLayers.map {
                                            if (it.id == id) it.copy(layerName = newName) else it
                                        }
                        )
                LayerType.STICKER ->
                        state.copy(
