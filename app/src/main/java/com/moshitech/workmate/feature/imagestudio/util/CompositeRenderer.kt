...
/**
 * Composite Renderer - Renders all layers onto base image This is the core component that enables
 * proper crop+layer interaction
 */
class CompositeRenderer(private val context: Context) {
    ...
    private fun renderAllLayers(
        ...
        state.stickerLayers.forEach { layer ->
            if (layer.isVisible) {
                allLayers.add(
                        layer.zIndex to
                                {
                                    renderStickerLayer(canvas, layer, bitmapScale, targetDensity)
                                }
                )
            }
        }
        ...
        allLayers.sortedBy { it.first }.forEach { (_, render) -> render() }
    }
    ...
    private fun renderTextLayer(canvas: Canvas, layer: TextLayer) {
        ...
        if (layer.isGlitch) {
            ...
            canvas.drawText(
                    layer.text,
                    textOriginX - glitchOffset,
                    textOriginY - glitchOffset,
                    redPaint
            )
        ...
        if (layer.reflectionOpacity > 0f) {
            ...
            // Since we stuck to positive Y going down, and we flipped the CONTENT relative to
            // Bottom,
            ...
            // Actually, simply translating Y by +offset works in the standard coordinate space IF
            // we apply it AFTER flip?
                    ...
                    Paint(paint).apply {
                        ...
                        shader =
                                LinearGradient(
                                        ...
                                        0f,
                                        textOriginY -
                                                textHeight, // Fading out towards the "bottom" of
                                        // the reflection (which is visually
                                        // top)
                                        intArrayOf(
                    ...
                    pathEffect =
                            when (layer.strokeStyle) {
                                ...
                                StrokeStyle.DASH_DOT ->
                                        DashPathEffect(floatArrayOf(30f, 15f, 5f, 15f), 0f)
    ...
    private fun renderStickerLayer(
            canvas: Canvas,
            layer: StickerLayer,
            bitmapScale: Float,
            targetDensity: Float?
        ...
        try {
            if (layer.resId != 0) {
                ...
                val inputStream =
                        context.contentResolver.openInputStream(android.net.Uri.parse(layer.uri))
        ...
        val density = targetDensity ?: context.resources.displayMetrics.density
        // Use bitmapScale to calculate the pixel size corresponding to 100dp on the screen
        val baseSize = (100f * density) / bitmapScale

        // Determine content bounds and size
        val contentWidth: Float
        val contentHeight: Float
        val finalFitScale: Float

        if (bitmap != null) {
            contentWidth = bitmap.width.toFloat()
            contentHeight = bitmap.height.toFloat()

            // Calculate Fit Scale for Bitmap (Fit within the baseSize box)
            val maxDim = kotlin.math.max(contentWidth, contentHeight)
            finalFitScale = if (maxDim > 0) baseSize / maxDim else 1f
        } else {
            // Text (Emoji) Case
            // UI uses fixed 80.sp inside 100.dp Box. centered.
            // We treat the "Content Box" as the full 100dp box (baseSize).
            contentWidth = baseSize
            contentHeight = baseSize

            // Text Size is 0.8 of the box size (80/100)
            textSize = baseSize * 0.8f

            // No scaling needed as we set textSize directly
            finalFitScale = 1f
        }
        ...
        // Apply Fit Scale (Used for bitmaps to fit box; 1f for text)
        canvas.scale(finalFitScale, finalFitScale)
        ...
        if (bitmap != null) {
                            ...
                            shader =
                                    android.graphics.LinearGradient(
                                            ...
                                            layer.gradientColors
                                                    .map {
                                                        // Apply alpha 0.8 to gradient colors to
                                                        // match screen
                                                        val a =
                                                                (android.graphics.Color.alpha(it) *
                                                                                0.8f)
                                                                        .toInt()
            ...
            val textPaint =
                    Paint(Paint.ANTI_ALIAS_FLAG).apply {
                        this.textSize = textSize // Use the calculated textSize
            ...
            // Draw text centered at 0,0 (Paint.Align.CENTER handles X, adjust Y for vertical
            // center)
        ...
        when (action) {
            is DrawAction.Path -> {
                ...
                if (isBlur) {
                    ...
                    sprayPaint.pathEffect =
                            ComposePathEffect(
                                    DiscretePathEffect(sprayJitter / 2f, sprayJitter),
                                    DashPathEffect(
                                            floatArrayOf(
                                                    action.path.strokeWidth * 0.1f,
                                                    action.path.strokeWidth * 1.5f
                                            ),
                                            0f
                                    )
                            )
            ...
            is DrawAction.Shape -> {
                ...
                when (action.shape) {
                    is Shape.Line -> {
                        canvas.drawLine(
                                action.shape.start.x,
                                action.shape.start.y,
                                action.shape.end.x,
                                action.shape.end.y,
                                paint
                        )
                    ...
                    is Shape.Circle -> {
                        canvas.drawCircle(
                                action.shape.center.x,
                                action.shape.center.y,
                                action.shape.radius,
                                paint
                        )
                    }
    ...
    private fun applyBlendMode(paint: Paint, blendMode: androidx.compose.ui.graphics.BlendMode) {
        if (android.os.Build.VERSION.SDK_INT >= 29) {
            paint.blendMode =
                    when (blendMode) {
                        androidx.compose.ui.graphics.BlendMode.Clear ->
                                android.graphics.BlendMode.CLEAR
                        ...
                        androidx.compose.ui.graphics.BlendMode.SrcOut ->
                                android.graphics.BlendMode.SRC_OUT
                        ...
                        androidx.compose.ui.graphics.BlendMode.DstAtop ->
                        ...
                        androidx.compose.ui.graphics.BlendMode.Overlay ->
                                android.graphics.BlendMode.OVERLAY
                        androidx.compose.ui.graphics.BlendMode.Darken ->
                                android.graphics.BlendMode.DARKEN
                        androidx.compose.ui.graphics.BlendMode.Lighten ->
                                android.graphics.BlendMode.LIGHTEN
                        ...
                        androidx.compose.ui.graphics.BlendMode.Exclusion ->
                                android.graphics.BlendMode.EXCLUSION
                        androidx.compose.ui.graphics.BlendMode.Multiply ->
                                android.graphics.BlendMode.MULTIPLY
                        ...
                        androidx.compose.ui.graphics.BlendMode.Color ->
                                android.graphics.BlendMode.COLOR
